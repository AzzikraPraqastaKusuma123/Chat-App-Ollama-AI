...
const HF_TOKEN = process.env.HF_TOKEN; // Ini akan mengambil nilai dari .env yang bernama HF_TOKEN
...
// Hanya mempertahankan format untuk Zephyr, dan mengoptimalkan system prompt-nya
function formatMessagesForZephyr(messagesArray, systemMessage = "Anda adalah asisten AI yang membantu, ramah, dan profesional. Jawablah dengan akurat, jelas, ringkas, dan tanpa typo dalam Bahasa Indonesia. Fokus pada inti pertanyaan.") {
    const relevantMessages = messagesArray.slice(-5); // Pertahankan 5 pesan terakhir
...
// Fungsi format lainnya dihapus karena modelnya tidak lagi digunakan.
...
app.post('/api/chat', async (req, res) => {
    ...
    const HF_ZEPHYR_TIMEOUT = 25000; // 25 detik (disesuaikan untuk menjadi satu-satunya model HF)
    ...
    // --- STRATEGI FALLBACK: 1. Zephyr (HF) -> 2. Ollama (Lokal) ---
    if (HF_TOKEN) {
        try {
            // --- COBA HUGGING FACE (Zephyr-7b-beta) ---
            console.log("    Mencoba LLM Hugging Face (Zephyr-7b-beta)...");
            ...
            // Menggunakan system prompt yang lebih baik untuk Zephyr
            const zephyrFormattedPrompt = formatMessagesForZephyr(messages, "Anda adalah asisten AI yang membantu, ramah, dan profesional. Jawablah dengan akurat, jelas, ringkas, dan tanpa typo dalam Bahasa Indonesia. Fokus pada inti pertanyaan.");
        ...
        primaryAIError = new Error("HF_TOKEN tidak ditemukan, melewati model Hugging Face.");
    }

    // --- FALLBACK KE OLLAMA (JIKA ZEPHYR GAGAL ATAU HF_TOKEN TIDAK ADA) ---
    if (!rawReplyContent && (!HF_TOKEN || primaryAIError)) { // Hanya coba Ollama jika belum ada respons dari HF atau HF_TOKEN tidak ada
        try {
            ...
            console.log(`    Model HF gagal atau tidak tersedia. Mencoba Ollama: ${ollamaModel} (Timeout: ${OLLAMA_TIMEOUT / 1000}s)...`);
            ...
            console.error(`    Gagal dari Ollama (${ollamaModel}) sebagai fallback: ${ollamaFinalError.message}`);
    ...
    if (finalReplyContent && HF_TOKEN) { // Hanya coba TTS jika ada HF_TOKEN, karena Gradio Client pakai HF_TOKEN
        try {
